...
**mini demo 的价值不在于「比 v2 好」**——综合能力 v2 明显更强；demo 的贡献是用严格
配对的实验证明：(a) 自研 PFN 路线端到端可行且便宜；(b) v2 存在一个可以靠「改训练
分布」补上的真实能力缺口（MNAR）。

| 场景 | 谁强 | 证据 | 该信几分 |
|---|---|---|---|
| 通用表格分类（真实数据） | **v2 明显强** | wine 60% 缺失 0.823 vs 0.746 | 可信——双方都分布外的公平场地 |
| 合成任务 | mini 略高 | 0.669 vs 0.652 | **不算数**——mini 的主场优势 |
| MCAR 鲁棒性 | 打平 | 双方 native ≈ mean-impute | 可信 |
| **MNAR** | **mini 显著强** | 0.706 vs 0.635（+7.1pt） | 可信但有机制匹配保留 |

实际价值：① de-risk 自研路线（全套脚手架可复用）；② 找到论文级缺口并给出因果证据
（结构相同、只差训练分布）；③ 工程决策依据——缺失随机则直接 v2 + NaN，信息性缺失
才值得微调/自研。

注意点：① 合成任务数字勿脱离主场优势标注引用；② MNAR 增益有机制匹配成分（训练与
评估同族机制），真实问卷需在先验里针对性建模；③ 利用缺失模式对**缺失机制漂移**敏感
（问卷改版会使学到的关联失效），生产中应监控缺失分布；④ 未测的廉价方案：给 v2 输入
手动拼接 0/1 缺失指示列；⑤ mini 先验窄（≤10 特征、≤4 类、≤160 行、纯数值），绝对
数字是方向性证据；⑥ 商用相关只能用 v2 权重。

下一步（按性价比）：真实数据上试 v2 + NaN / v2 + 指示列 → 需要时官方 finetuning +
缺失机制模拟（方案 C）→ 领域结构超出 v2 先验时才完整自研（方案 D）。
